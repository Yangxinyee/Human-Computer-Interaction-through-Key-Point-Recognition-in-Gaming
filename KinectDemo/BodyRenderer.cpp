...
HRESULT BodyRenderer::EnsureResources()
{
    ...
    if (pD2DFactory && !pRenderTarget)
    {
        ...
        pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Red, 0.3f), &pBrushRec);
    }
...
void BodyRenderer::DrawRec(Joint* pJoints)
{
    Joint neck = pJoints[JointType_Neck];
    ...
    CameraSpacePoint cameraUpLeft = { 0.45, 0.05, 1.3 };
    CameraSpacePoint cameraDownRight = { 1.5, -0.85, 1.3 };
    pCoordinateMapper->MapCameraPointToDepthSpace(cameraUpLeft, &depthUpLeft);
    pCoordinateMapper->MapCameraPointToDepthSpace(cameraDownRight, &depthDownRight);
    // 画虚拟电脑屏幕
    pRenderTarget->DrawRectangle(D2D1::RectF(depthUpLeft.X, depthUpLeft.Y, depthDownRight.X, depthDownRight.Y), pBrushRec, 3.0f);
}
